...
void DebugAnimationModel::Move() {
	...
	// velocityに反映
	baseInfo_.velocity = info_.direction * info_.spped;

	/// ===移動方向に沿って回転=== ///
	// 方向が変更されたら
	if (Length(info_.direction) > 0.01f) {
		// 現在のYaw角（Y軸の回転）を取得
		float currentYaw = Math::GetYAngle(transform_.rotate);
		...
		// Quaternionに再変換
		transform_.rotate = Math::MakeRotateAxisAngle({ 0, 1, 0 }, easedYaw);
	}
}
